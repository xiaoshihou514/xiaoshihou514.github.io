...
值得注意的是，$\Lambda_{NR}$系统仍然有着诸多限制，虽然我们没有提，但是这个类型系统理论上仍然不允许多态（虽然类型推导算法实现了类似的效果）。我们会在下一篇博客里通过ML类型——一种更加优雅的类型系统——引入多态，它是Hindley Milner在已有的$\Lambda_N$、$\Lambda_{NR}$基础上构建的类型系统，是函数式编程在类型系统上的一座里程碑。
